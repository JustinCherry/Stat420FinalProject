...
full_model = lm(WL ~ . - (School + City + State), data = school_trn_data)
#full_model

significant_model = lm(WL ~ From + To + SRS + SOS + CTRN + NCAA, data = school_trn_data)
#significant_model
...
diagnostics(significant_model)
```

```{r}
#influential points (leverage)
big_mod_cd = cooks.distance(significant_model)
sum(big_mod_cd > 4 / length(significant_model))
```
