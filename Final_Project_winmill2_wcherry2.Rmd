...
- We've made slight alterations to the original dataset including:
  ...
  - separating "City,State" into "City" and "State". We wanted to be able to use city and state separately in our model.
  - removing any rows that introduced null values.
  - converting "state" into a factor. This was one of our categorical variables, so we needed the factor.
...
- The model we decided was best for our test was the model using the predictors - From, To, Yrs, SRS, SOS, CTRN, AP, CREG, and NC (Code for model included below). We started off with the full_model. With the full_model we performed a backwards AIC and BIC. However, in both of these results was chosen the predictors G and L... Knowing what those are (Games played and Loses), you would be able to calculate exactly what the win-loss ratio would be. We figured that this was probably not the best model to choose because it would make it so that we couldn't figure out if the other predictors were useful or not in determining win-loss ratio. We decided not to use either G or L, and that is why we decided to perform a forward BIC. We started from a null_model, and then made it so that its scope was all of the predictors except a few... two of those being W or L (Wins or Losses). We did this to make sure neither of those were selected as predictors. We then ran the forward BIC and got better model. However, it still wasn't where we wanted it. So we decided to throw in some interactions to see if we could get better results in the Fitted vs Residuals plot, the QQ plot, and the Shapiro test. After making some modifications, we ended up with the below model that we were pleased with (It ended up not needed W, L, or G in order to get a good model).

```{r}
modified_forward_BIC = lm(WL ~ I(SRS ^ 2) + SOS + I(To ^ 2) + From + Yrs + CTRN + I(AP ^ 2) + I(CREG ^ 2) + I(NC ^ 2), data = school_trn_data)
```

- Below you can see the results of some of the tests we performed. In the diagnostics function, we create a Fitted vs Residuals Plot, a QQ plot, and then we do a Shapiro test and make a decision based on the p-value returned. This diagnostic function was very useful in order for us to keep improving the model.

```{r}
diagnostics(modified_forward_BIC)
```

- Also, we compared the predicted vs actual data points to see how accurate/well our model did. This makes a prediction using the school_tst_data for each of the schools. It then takes the actual values and compared them in a plot against the predicted values. This was helpful to us so that we could see if the points followed a pattern and if so, how accurate they were.
...
- Say someone going to Brigham Young University (Provo - close to where we are from) wanted to see a prediction for about how well the Cougars will do in a season. Having the following information - SRS = 7.56, SOS = 3.54, To = 2019, From = 1903, Yrs = 117, CTRN = 3, AP = 10, CREG = 25, NC = 0... We could make a prediction for what their win-loss ratio will be using our model.

```{r}
cougar_prediction = data.frame(SRS = 7.56,
                               SOS = 3.54,
                               To = 2019,
                               From = 1903,
                               Yrs = 117,
...
- As you can see, with that information the predicted win-loss ratio was `r cougars$predicted` (`r format(round((cougars$predicted)*100, 2), nsmall = 2)`%) and the actual win-loss ratio was `r cougars$actual` (`r format(round((cougars$actual)*100, 2), nsmall = 2)`%). The prediction was off by `r cougars$actual-cougars$predicted` which is `r format(round((cougars$actual-cougars$predicted)*100, 2), nsmall = 2)`%... That is pretty close. The Cougar fan would then know that their team will probably win a little more than half of their games.

**Prediction for The University of Illinois**

- Another example using the data from The University of Illinois - SRS = 12.76, SOS = 7.65, To = 2019, From = 1906, Yrs = 114, CTRN = 2, AP = 26, CREG = 17, NC = 0.

```{r}
illiniwek_prediction = data.frame(SRS = 12.76,
                                  SOS = 7.65,
                                  To = 2019,
                                  From = 1906,
                                  Yrs = 114,
...
- With that information the predicted win-loss ratio was `r illiniweks$predicted` (`r format(round((illiniweks$predicted)*100, 2), nsmall = 2)`%) and the actual win-loss ratio was `r illiniweks$actual` (`r format(round((illiniweks$actual)*100, 2), nsmall = 2)`%). The prediction was off by `r illiniweks$actual-illiniweks$predicted` which is `r format(round((illiniweks$actual-illiniweks$predicted)*100, 2), nsmall = 2)`%... Again, that is pretty close and is within 10%. The Illinois fan would then know that their team has a good chance to win a little more than half of their games as well.

**Another Prediction**

- In this one, we just wanted to show a prediction that was very close.

- Boston University - SRS = -6.84, SOS = -5.48, To = 2019, From = 1916, Yrs = 93, CTRN = 6, AP = 0, CREG = 8, NC = 0

```{r}
other_prediction = data.frame(SRS = -6.84,
                   SOS = -5.48,
                   To = 2019,
                   From = 1916,
                   Yrs = 93,
...
- With this other information, the predicted win-loss ratio was `r another$predicted` (`r format(round((another$predicted)*100, 2), nsmall = 2)`%) and the actual win-loss ratio was `r another$actual` (`r format(round((another$actual)*100, 2), nsmall = 2)`%). The prediction was off by `r another$predicted-another$actual` which is `r format(round((another$predicted-another$actual)*100, 2), nsmall = 2)`%. 

- So you can see that while these predictions vary between really close and about 10% off, they are fairly consistent and can give a pretty good estimate as to what a teams win-loss ratio will be.

### Discussion

- After analyzing the different variables, we came to the conclusion that the best model was using From, To, Yrs, SRS, SOS, CTRN, AP, CREG, and NC as the predictors (some of them considering interactions). These variables give information including when a team started playing in the seasons, how long they have been playing, the strength of their schedule (how difficult the teams are they were playing), and even some of the results in championships (a count of championships won). Using these variables we were able to create a model and create a graph that would show how close we could predict a teams win-loss ration based on these predictors. Looking at the graph you can see that the predictions aren't perfect, but they can give a pretty good guess as to what a teams win-loss ration will be. Just by looking at the graph you can see that with those predictors, it was able to guess within about 10% their win-loss ratios. I feel like this is pretty good considering all the other models we looked at and how bad they seemed to be.
